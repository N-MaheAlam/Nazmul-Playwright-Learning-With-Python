...
def test_second_browser_check_shortcut(page: Page):
    page.goto("https://www.facebook.com/")
    print("\nHello, This is a  Playwright code ")
...
def test_rahul_shetty_academy_login_page_input_values(page: Page):
    ...
    # it has a label tag (inspect element then you can see). Then, to use the "get_by_label" it should
    # follow 2 rules and any one of this

    # (((1))) the input tag should be inside the label tag such as
    # <label> Password <input type="password" /> </label>
    # (value for example "("rahulshettyacademy")" should be inside the label)
    # the edit box where we put the value could be an input tag which refers to HTML.

    # or (((2)))
    # the label should have a "for" attribute that will be used
    # as "id" in input tag if input is written separately outside label. such as

    # <label for="username" class="text-white">Username:</label>
    # <input type="text" name="username" id="username" class="form-control">
    # you see the "label" has an attribute "for" which has the same value as the "id" attribute of
    # "input" tag
    page.get_by_label("Username:").fill("rahulshettyacademy")
    page.get_by_label("Password:").fill("Learning@830$3mK2")

    # when we want to select something from a predefined box we call it "combobox" and in
    # "get_by_role" we provide "combobox" and the use "select_option". Inside the "select_option"
    # argument we provide the "value" attributes value
    # If you inspect element you will see the "value" tag in html which has a
    # value "consult" and this we will put in our "select_option". Remember, in web page, it is showing
    # as "Consultant" but in html "value" tag its "consult", so we will use the "consult"
